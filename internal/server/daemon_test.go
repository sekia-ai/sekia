...
func TestEndToEnd(t *testing.T) {
	...
	}, "test-agent", "0.2.3", []string{"testing"}, []string{"ping"}, logger)
	if err != nil {
	...
	if agents.Agents[0].Version != "0.2.3" {
		t.Fatalf("expected agent version 0.2.3, got %s", agents.Agents[0].Version)
	}
...
`
	...
	testAgent, err := agent.New(agent.Config{
		NATSUrl:  d.NATSClientURL(),
		NATSOpts: d.NATSConnectOpts(),
	}, "test-agent", "0.2.3", []string{"testing"}, []string{"handle"}, zerolog.New(os.Stderr))
	if err != nil {
		t.Fatalf("create test agent: %v", err)
	}
	defer testAgent.Close()
...
`
	...
	testAgent, err := agent.New(agent.Config{
		NATSUrl:  d.NATSClientURL(),
		NATSOpts: d.NATSConnectOpts(),
	}, "test-agent", "0.2.3", []string{"testing"}, []string{"label"}, logger)
	if err != nil {
		t.Fatalf("create test agent: %v", err)
	}
	defer testAgent.Close()
